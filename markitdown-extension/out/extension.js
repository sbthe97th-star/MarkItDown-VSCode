...
function activate(context) {
    ...
    context.subscriptions.push(vscode.languages.registerCodeLensProvider('markdown', codeLensProvider));
    ...
    context.subscriptions.push(vscode.commands.registerCommand('type', (args) => typeCommandInterceptor.interceptType(args)));
}
function deactivate() {
...
function formatTableLines(lines) {
    ...
    const maxWidths = new Array(maxColumns).fill(0);
 ...
 */
async function copyCodeBlock(codeBlockInfo) {
    const editor = vscode.window.activeTextEditor;
    if (!editor) {
        return;
    }
 ...
 */
class CodeBlockCodeLensProvider {
    provideCodeLenses(document, token) {
        ...
        while ((match = codeBlockRegex.exec(text)) !== null) {
            ...
            const codeLens = new vscode.CodeLens(new vscode.Range(codeLensPosition, codeLensPosition), {
                title: '📋 Copy',
 ...
 */
class AdmonitionFoldingProvider {
...
class TypeCommandInterceptor {
    ...
    async interceptType(args) {
        const config = vscode.workspace.getConfiguration('markitdown');
        const enableAutoRow = config.get('enableAutoRow', false);
